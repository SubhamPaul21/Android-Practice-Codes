...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray featuresJsonArrayList = rootJsonObject.optJSONArray("features");

//            Log.d("featuresJsonArrayList",String.valueOf(featuresJsonArrayList));

            assert featuresJsonArrayList != null;
            for (int i = 0; i < featuresJsonArrayList.length(); i++) {
                JSONObject featuresJsonObject = featuresJsonArrayList.getJSONObject(i);

//                Log.d("propertiesJsonObject",String.valueOf(featuresJsonObject));
...
//                Log.d("magnitude" + i, "extractEarthquakes: " + magnitude);

                String location = propertiesJsonObject.optString("place");
//                Log.d("location" + i, "extractEarthquakes: " + location);
                if (location.contains("of")) {
                    indexValue = location.indexOf("of") + 2;
                    offset = location.substring(0, indexValue);
                    location = location.substring(indexValue).trim();
                } else {
                    location = "Near the " + location;
                    indexValue = location.indexOf("the") + 3;
                    offset = location.substring(0,indexValue);
                    location = location.substring(indexValue).trim();

                }

                long milliseconds = propertiesJsonObject.optLong("time");

                String url = propertiesJsonObject.optString("url");

//                Log.d("time" + i, "extractEarthquakes: " + time);

                earthquakes.add(new Earthquake(magnitude,offset,location,milliseconds,url));
